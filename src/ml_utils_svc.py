"""Utilities for machine learning pipelines used in the evictions-learn project, specific version for running LinearSVC model.
   NOTE: for LinearSVC, models cannot be trained on a single county because the economic data is at the county level, yielding
   distance scores that are the same across all observations, which produces an error during normalization. The cursor must be updated
   to select more than a single county to run this file without error."""
import os
...
def main():
    ...
    phase1 = {"name": "phase1", "models": [
        'SVC'], "grid": GRID_1, "latest_split": False}
