...
def sum_stat_table(cur = db_connect()[0]):
	...
	stddev_row = ["std"] + list(cur.fetchone())

	df = pd.DataFrame([avg_row, min_row, max_row, stddev_row])
	df.columns = cols
